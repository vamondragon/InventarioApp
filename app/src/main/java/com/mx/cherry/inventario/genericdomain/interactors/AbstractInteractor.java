...
/**
 * <p/>
 * This abstract class implements some common methods for all interactors. Cancelling an x, check if its running
 * and finishing an x has mostly the same code throughout so that is why this class was created. Field methods
 * are declared volatile as we might use these methods from different threads (mainly from UI).
 * <p/>
 * For example, when an activity is getting destroyed then we should probably cancel an x
...
public abstract class AbstractInteractor implements Interactor {
    ...
    /**
     * This method contains the actual business logic of the x. It SHOULD NOT BE USED DIRECTLY but, instead, a
     * developer should call the execute() method of an x to make sure the operation is done on a background thread.
     * <p/>
    ...
    public void execute() {
        ...
        // mark this x as running
        this.mIsRunning = true;

        // start running this x in a background thread
